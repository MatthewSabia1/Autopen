...
export default function ProjectsPage() {
  ...
  const handleDelete = async (id: string) => {
    console.log("Delete handler called for project ID:", id);
    ...
    try {
      ...
      console.log("Delete action in progress for:", project.title);
      ...
      const success = await deleteProduct(id, project.source);
      if (!success) throw new Error("Failed to delete project");
      
      console.log("Project successfully deleted:", project.title);
  ...
  const handleEdit = (id: string) => {
    ...
    console.log("Navigating to edit page for:", project.title);
  ...
  const handleView = (id: string) => {
    ...
    if (!project) {
      console.warn("Project not found in state for viewing:", id);
      console.log("Navigating to project detail by ID only");
      navigate(`/products/${id}`);
      return;
    }
    
    console.log("Navigating to project detail:", project.title);
    navigate(`/products/${id}`);
  };
  ...
  const handleContinue = (id: string) => {
    const project = products.find(p => p.id === id);
    if (!project) {
      console.error("Project not found for continue action:", id);
      return;
    }
    
    if (project.status === 'complete' || project.status === 'published') {
      console.log("Project already complete/published, ignoring continue action");
      return;
    }
    ...
    if (project.type === 'ebook') {
      const resumeStep = project.metadata?.workflow_step ||
        (project.status === 'in_progress' ? 'ebook-writing' : 
         project.status === 'draft' ? 'brain-dump' : null);
      ...
      if (resumeStep) {
        ...
        if (project.project_id) {
          console.log(`Navigating to workflow/${project.project_id} to resume at ${resumeStep}`);
    ...
    console.log("Navigating to creator for continue action:", project.title);
      ...
      <DashboardLayout activeTab="Projects">
        <div className="flex h-[60vh] w-full items-center justify-center">
          <div className="flex flex-col items-center">
            ...
            <p className="text-ink-light font-serif text-base mt-4">Loading your projects...</p>
          </div>
        </div>
      </DashboardLayout>
    );
      ...
      <DashboardLayout activeTab="Projects">
        <div className="flex flex-col h-[60vh] w-full items-center justify-center">
          ...
          <div className="text-ink-dark dark:text-ink-dark font-serif text-center max-w-lg px-4 mb-4">
            <h3 className="text-xl font-display mb-2">Couldn't Load Projects</h3>
      ...
      <div className="space-y-6 animate-fade-in">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-2">
          <div>
            ...
            <p className="font-serif text-ink-light dark:text-ink-light/80 text-sm">Create, organize, and manage your content projects</p>
          </div>
          ...
          <CardContent className="p-0">
            {!products || products.length === 0 ? (
              <div className="p-16 text-center">
                ...
                <h3 className="text-xl font-display text-[#333333] dark:text-ink-dark mb-3">No projects found</h3>
                <p className="text-[#666666] dark:text-ink-light font-serif mb-8 max-w-md mx-auto">
                  Create your first project using the AI Creator or capture your initial ideas with a Brain Dump.
                </p>
              ...
              <div className="p-16 text-center">
                ...
                <h3 className="text-xl font-display text-[#333333] dark:text-ink-dark mb-2">No matching projects</h3>
                      ...
                      >
                        <TableCell className="font-serif text-[#333333] dark:text-ink-dark py-4 px-5 font-medium">
                          <div className="flex items-center">
                            <div className="w-9 h-9 rounded-full bg-[#F5F5F5] dark:bg-accent-tertiary/20 flex items-center justify-center mr-3 group-hover:bg-[#738996]/10 dark:group-hover:bg-accent-primary/20 transition-colors duration-300">
                              {getProductIcon(project.type)}
                            </div>
                            <span className="group-hover:text-[#738996] dark:group-hover:text-accent-primary transition-colors duration-300 truncate max-w-[250px]">{project.title}</span>
                          </div>
                        </TableCell>
                        <TableCell className="font-serif text-[#666666] dark:text-ink-light py-4">
                          <Badge className="font-serif text-xs px-2 py-0.5 bg-[#F9F5ED] dark:bg-accent-yellow/15 text-[#ccb595] dark:text-accent-yellow border-[#ccb595] dark:border-accent-yellow/50 border shadow-xs dark:shadow-none group-hover:bg-white/90 dark:group-hover:bg-card/90 transition-colors duration-300">
                            {project.type === 'ebook' ? 'eBook' : 
                            project.type === 'brain_dump' ? 'Brain Dump' : 
                            project.type.charAt(0).toUpperCase() + project.type.slice(1)}
                          </Badge>
                        </TableCell>
                        <TableCell className="py-4">{renderStatusBadge(project.status)}</TableCell>
                        ...
                        <TableCell className="text-right py-4 pr-6" onClick={(e) => e.stopPropagation()}>
                          <DropdownMenu open={openMenuId === project.id} onOpenChange={(open) => {
                            if (open) {
                              setOpenMenuId(project.id);
                            } else if (!actionInProgress || actionInProgress.id !== project.id) {
                              setOpenMenuId(null);
                            }
                          }}>
                            <DropdownMenuTrigger asChild>
                              <Button 
                                ...
                                aria-label="Actions for this project"
                                onClick={(e) => {
                                  e.preventDefault();
                                  e.stopPropagation();
                            ...
                            <DropdownMenuContent 
                              ...
                              onPointerDownOutside={(e) => {
                                if (actionInProgress && actionInProgress.id === project.id) {
                                  e.preventDefault();
                                }
                              }}
                            >
                              <DropdownMenuItem
                                ...
                                disabled={actionInProgress?.id === project.id}
                                onClick={(e) => {
                                  e.preventDefault();
                                  e.stopPropagation();
                                ...
                                onSelect={(e) => {
                                  e.preventDefault();
                                  if (actionInProgress?.id === project.id) {
                                    e.preventDefault();
                                  }
                                }}
                              >
                                {actionInProgress?.id === project.id && actionInProgress?.action === 'view' ? (
                                  <>
                              ...
                              >
                                {actionInProgress?.id === project.id && actionInProgress?.action === 'edit' ? (
                                  <>
                                    <Loader2 className="mr-2 h-4 w-4 animate-spin text-[#738996] dark:text-accent-primary" />
                              ...
                              {project.status !== 'complete' && project.status !== 'published' && (
                                <DropdownMenuItem
                                  className="font-serif cursor-pointer text-[#333333] dark:text-ink-dark hover:bg-[#F5F5F5] dark:hover:bg-accent-tertiary/20 rounded-md p-2.5 transition-all duration-300 focus:bg-[#F5F5F5] dark:focus:bg-accent-tertiary/20 focus:outline-none"
                                  disabled={actionInProgress?.id === project.id}
                                  onClick={(e) => {
                                ...
                                >
                                  {actionInProgress?.id === project.id && actionInProgress?.action === 'continue' ? (
                                    <>
                                      <Loader2 className="mr-2 h-4 w-4 animate-spin text-[#738996] dark:text-accent-primary" />
                              ...
                              >
                                {actionInProgress?.id === project.id && actionInProgress?.action === 'delete' ? (
                                  <>
            ...
            {filteredProjects.length > 0 && (
              <div className="px-5 py-4 text-center border-t border-[#F0F0F0] dark:border-accent-tertiary/30 bg-[#FAF9F5] dark:bg-card/80 flex items-center justify-between">
                <p className="font-serif text-xs text-[#888888] dark:text-ink-light/70">Showing {filteredProjects.length} {filteredProjects.length === 1 ? 'project' : 'projects'}</p>
                <Button 
                  variant="link" 
                  size="sm" 
